...
const getUserMeterReading = async(user_id,meter_number)=>{
  const query = `select DATE_FORMAT(mr.reading_date,'%Y-%m-%d') as reading_date ,mr.reading_value,b.billing_amount,b.is_paid from user_details ud join user_meter_mapping umm on ud.id  = umm.user_id 
...
where ud.id  = ? and m.meter_number = ? AND ud.is_deleted = 0  and mr.is_deleted =0`;
